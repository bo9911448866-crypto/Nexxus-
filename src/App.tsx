...
import { ScheduleTrackerTab } from './components/ScheduleTrackerTab.tsx';
...
export default function App() {
  ...
  // Update telemetry when active game or tab changes
  useEffect(() => {
    if (activeGame) {
      telemetry.updateGame(activeGame.title);
    } else if (activeTab === 'schedule') {
      telemetry.updateGame('Class Schedule & Tracker');
    } else {
  ...
  return (
    <div className="min-h-screen bg-black text-zinc-100 font-sans selection:bg-white selection:text-black">
      ...
      {activeGame ? (
        ...
        <ScheduleTrackerTab onBackToGames={() => setActiveTab('games')} />
      ) : (
